...
import { createTracker } from '../src/trackers/index.mjs';
...
describe('createTracker (factory)', () => {
  it('throws when api_key_env_var is not set in environment', () => {
    ...
    expect(() => createTracker(cfg())).toThrow(/TEST_TRACKER_KEY/);
  });
  ...
  it('throws on unknown tracker kind', () => {
    process.env.TEST_TRACKER_KEY = 'fake-key';
    expect(() => createTracker(cfg({ kind: 'jira' }))).toThrow(/Unknown tracker kind/);
    delete process.env.TEST_TRACKER_KEY;
  });
  ...
  it('parses terminal_states JSON string', () => {
    ...
    expect(() => createTracker(cfg({
      kind: 'linear',
      terminal_states: '["Done","Duplicate"]',
    }))).not.toThrow();
    delete process.env.TEST_TRACKER_KEY;
  });
