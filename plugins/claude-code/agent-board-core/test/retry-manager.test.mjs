...
import { computeBackoffMs, scheduleRetry, listRetryStates } from '../src/retry-manager.mjs';
...
function makeDb(rows = []) {
  ...
  return {
    _rows: insertedRows,
    prepare(sql) {
      const isInsert = /^INSERT/.test(sql.trim());
      return {
        run(...args) {
...
describe('scheduleRetry', () => {
  ...
  it('preserves error string in retry_state row', () => {
    const db = makeDb();
    scheduleRetry(db, { runId: 'r1', taskId: 't1', role: 'worker', attempt: 1, error: 'API timeout' });
    const retryRow = db._rows.find(r => r.sql.includes('retry_state'));
