 ...
 * @property {'timeout'|'error'} [errorKind] - present when status='failed'; 'timeout' marks TurnTimeout aborts so executor can skip auto-retry.
 */
  ...
  async run() {
    ...
    try {
      ...
      const isTimeout = err?.name === 'TimeoutError'
        || /Turn timed out after \d+ms/.test(err?.message || '');
    ...
    try {
      for await (const msg of q) {
        ...
        if (msg.type === 'system' && msg.subtype === 'init' && msg.session_id) {
          ...
          if (msg.model) { result.model = msg.model; this.#partial.model = msg.model; }
        }
        ...
        if (msg.type === 'assistant' && msg.message?.model && !result.model) {
          ...
          this.#partial.model = msg.message.model;
        }
