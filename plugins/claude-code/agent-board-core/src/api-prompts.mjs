...
const KIND_WHITELIST = new Set(['role', 'skill']);
const ID_RE = /^[a-zA-Z0-9_-]{1,64}$/;

function candidatePaths(kind, id) {
  return [
...
export async function handlePrompts(req, res, url) {
  if (req.method !== 'GET') return null;
  const m = url.pathname.match(/^\/api\/prompts\/([a-z]+)\/([^/]+)$/);
  if (!m) return null;

  const kind = m[1];
  const id = decodeURIComponent(m[2]);
  if (!KIND_WHITELIST.has(kind)) return json(res, 400, { error: 'unknown kind' });
  if (!ID_RE.test(id)) return json(res, 400, { error: 'invalid id' });
  ...
  return json(res, 404, { kind, id, content: null, error: 'prompt not found' });
}
