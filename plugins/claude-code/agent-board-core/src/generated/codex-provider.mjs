...
function readCodexConfig(projectDir = null) {
  ...
  const projectCfg = projectDir !== null ? readCodexConfigFile(join(projectDir, ".codex", "config.toml")) : { model: null, mcpServers: {} };
  return {
...
function normalizeClaudeServer(s) {
  ...
  if (rawArgs !== void 0 && rawArgs !== null && !Array.isArray(rawArgs) && typeof rawArgs === "object") {
    const argsObj = rawArgs;
    const keys = Object.keys(argsObj).filter((key) => /^\d+$/.test(key)).sort((a, b) => parseInt(a, 10) - parseInt(b, 10));
...
function codexReferencedEnvKeys(projectDir = null) {
  ...
  for (const server of Object.values(cfg.mcpServers)) {
    if (server.bearer_token_env_var !== void 0)
...
var CodexRunner = class {
  ...
  async run() {
    ...
    try {
      ...
      const isTimeout = error.name === "TimeoutError" || /timed out after \d+ms/i.test(error.message);
      return {
  ...
  async execute(turnSignal) {
    ...
    const userMcpEntries = Object.entries(mcpServers ?? {}).filter(
      ([name]) => name !== "abrun"
    );
    const bridged = {
    ...
    for (const [name, server] of Object.entries(bridged)) {
      ...
      if (server.command) {
        configArgs.push(
          "-c",
          `${pathKey}.command=${quoteTomlString(server.command)}`
        );
    ...
    const fullPrompt = systemPrompt.trim().length > 0 ? `${systemPrompt}
...
${prompt}` : prompt;
    ...
    if (existsSync2(lastMessagePath)) {
      try {
        ...
        if (text)
          sessionLog?.info(
            { runId, lastMessage: text.slice(0, 500) },
            "Codex final message"
          );
      } catch {
      }
  ...
  handleJsonLine(line, result, usage, onEvent, sessionLog, runId) {
    ...
    const sessionIdCandidate = typeof obj.session_id === "string" && obj.session_id.length > 0 ? obj.session_id : typeof obj.thread_id === "string" && obj.thread_id.length > 0 ? obj.thread_id : null;
...
function codexSandboxArgs(sandbox) {
  if (sandbox?.restrictToWorkspace === false)
    throw new Error(
      "Codex provider requires workspace-restricted sandbox execution"
    );
...
function extractUsage(obj) {
  ...
  const responseObj = responseField !== null && typeof responseField === "object" ? responseField : null;
  const candidate = obj.usage ?? (dataObj !== null ? dataObj.usage : void 0) ?? (resultObj !== null ? resultObj.usage : void 0) ?? (responseObj !== null ? responseObj.usage : void 0);
  ...
  return {
    ...
    cache_read_tokens: num("cache_read_tokens") || num("cacheReadTokens") || num("cached_input_tokens")
  };
}
...
function createCodexProviderAdapter(args) {
  return {
    ...
    resume: {
      interactive: true,
      command: (sessionId, repoPath) => args.buildResumeCommand(codexProviderManifest.id, sessionId, repoPath)
    },
    async run(ctx) {
