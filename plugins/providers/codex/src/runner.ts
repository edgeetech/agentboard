...
import { buildCodexChildEnv } from "./environment.ts";
...
export class CodexRunner {
  ...
  private async execute(turnSignal: AbortSignal): Promise<ProviderRunResult> {
    const {
      prompt,
      systemPrompt,
      cwd,
...
function codexSandboxArgs(
  sandbox: ProviderSandboxPolicy | undefined,
): string[] {
...
function defaultRunConfigDir(): string {
  const dataDir =
...
function extractUsage(obj: Record<string, unknown>): ExtractedUsage | null {
  ...
  return {
    ...
    cache_creation_tokens:
      num("cache_creation_tokens") || num("cacheCreationTokens"),
    cache_read_tokens:
