...
```
agentboard/
  agent-board-core/       Node server, executor, role prompts, React UI
  plugins/claude-code/    Thin plugin: manifest, skills, stdio MCP, SessionStart hook
  .claude-plugin/
...
```bash
cd agent-board-core/ui && npm install && npm run build
```
