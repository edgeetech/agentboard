...
1. **HTTP MCP key `abrun`** (`plugins/claude-code/agent-board-core/src/api-mcp.ts`) — consumed by **spawned headless `claude -p` runs** via a per-run tmp `--mcp-config` file. Exposes:
   - **Run lifecycle:** `get_task, update_task, add_comment, finish_run, add_heartbeat`.
   ...
   Auth is `Authorization: Bearer <run_token>` ONLY — the server's own Bearer token is never accepted on `/mcp` (see "Security model" below). `run_token` is minted by the executor's own `claimRun()` (`src/repo.ts`) when it claims a queued run, rotates per claim, and scopes every call to that run's own task. `list_queue` and `claim_run` are intentionally **not** exposed as MCP tools — the executor claims runs itself, so no spawned agent can list the queue or claim (and thereby obtain the `run_token` of) a run it wasn't spawned for. Speaks JSON-RPC 2.0.
...
- 32-byte hex token in `~/.agentboard/config.json` (0600 on Unix, `icacls` ACL on Windows). Compared with `crypto.timingSafeEqual` (`src/auth.ts::constantTimeEqual`), never `===`.
...
- **`/mcp` never accepts the server Bearer token** — spawned agents (Claude, Codex, Copilot, council members) authenticate every call with `Authorization: Bearer <run_token>` of a RUNNING run, resolved via `getDbForRunToken` (`src/project-registry.ts`, in-memory `run_token → project code` cache populated at claim time, DB-verified on every call). The server token would let a compromised or prompt-injected agent call any REST endpoint as the human (approve its own work, flip `allow_destructive_tools`, delete projects) — it is only ever handed to the browser UI and the stdio plugin MCP (`plugins/claude-code/mcp/agentboard.mjs`), neither of which is spawned/untrusted content. `src/executor.ts` passes each run's own `run_token` (never the server token) into the SDK `mcpServers.abrun` header, the PreToolUse hook/tool-gate params (`src/run-hooks.ts`), and `ProviderRuntimeContext.mcpBearerToken` (Codex forwards it verbatim as its `abrun` bearer).
- A `run_token` argument inside MCP `tools/call` params must match the header token exactly, or the call is rejected — an agent cannot act on a different run than the one whose header it authenticated with.
- `list_queue` / `claim_run` are not exposed as MCP tools. The executor claims runs itself (`src/repo.ts::claimRun`); a worker never needs, and must never be able, to list the queue or mint another run's token.
- Tool policy (`src/tool-policy.ts`) evaluates the **full, untruncated** shell command, splitting on `&&`/`||`/`;`/`|`/newlines (quote-aware) so compound commands (`ls && git commit`, `echo hi | xargs rm`, `git -C . push`, `find . -delete`) can't slip past a single-pattern check. Inline code evaluation (`node -e/--eval/-p`, `python -c`) is always blocked, independent of `allow_destructive_tools`. Any tool target referencing the AgentBoard data dir (`~/.agentboard` — holds `config.json`/token, project DBs, run logs), in any slash/tilde/env-var spelling, is denied outright. `sandbox.restrictToWorkspace: true` (`src/provider-runtime.ts`) is currently declarative only — no code path enforces it yet.
- Postflight required-comment checks (`src/postflight.ts::checkPostflight`) are scoped to comments authored by the run's own role with `created_at >= run.started_at` — a rework run cannot inherit credit for a prior run's `DEV_COMPLETED`/`REVIEW_VERDICT`, and a human comment can't satisfy a role-specific gate. `evaluateRunCompletion()` combines the required-comment check with the inner phase gate and is used by both the MCP `finish_run` path and the executor's natural-end-of-turn path.
...
- **All new scripts are TypeScript (`.ts` / `.tsx`).** This repo migrated off `.mjs` for server, plugin, hook, and installer code; new files must follow suit. The remaining `.mjs` files in `plugins/claude-code/hooks/session/` are legacy bundles and pre-port hook entry-points — leave them alone unless rewriting them, but do not introduce new `.mjs` files. Plugin hook commands and skill instructions launch `.ts` scripts via `node --experimental-strip-types --no-warnings <path>.ts`.
...
- **Child env is whitelisted, not inherited.** `src/child-env.ts::buildChildEnv` ships only PATH/HOME/USER/LANG/TZ/proxy+CA vars + Claude-auth vars (`ANTHROPIC_API_KEY`, `CLAUDE_CODE_OAUTH_TOKEN`, etc.) on POSIX; Windows adds USERPROFILE/APPDATA/LOCALAPPDATA/SYSTEM*/HOMEDRIVE/HOMEPATH/windir/PSModulePath and friends. AWS, SSH, GCP, cloud SDK vars are dropped. `executor.ts` builds this env once per run and passes it through `ProviderRuntimeContext.env` — Claude/Copilot/Codex runners consume that field directly rather than reading `process.env` themselves; if you add a new env var a CLI needs, add it to `child-env.ts` (and its duplicate whitelist in `plugins/providers/codex/src/environment.ts`) — never `{...process.env}` in a runner.
- **Per-provider `auth_mode`** (`project.auth_config_json`, parsed via `agent-config.ts::parseAuthConfig`/`resolveAuthMode`) is `'subscription' | 'api_key' | 'auto'` per provider. `'subscription'` strips that provider's API-key env vars from the run's env (`stripApiKeyEnvVars`) so a stray key can't silently override an interactive login and bill the API; `'api_key'` fails the run fast (before spawning) if the required var is absent; `'auto'` (default) is unchanged — whatever the CLI resolves with the whitelisted env. `agent_run.auth_source` records what the provider actually used (Claude SDK `system.init.apiKeySource`); `pricing.ts::isEstimatedCost` derives `cost_is_estimate` from it for the cost/run APIs.
- **`claimRun()` (`src/repo.ts`) returns `run_token` exactly once, to the queued→running CAS winner — and only the executor calls it, never an MCP tool.** There is no "already-claimed" retry path; a subsequent claim attempt on a running run fails its CAS. This is deliberate; don't add a "convenience" retry, and don't re-add `claim_run`/`list_queue` to the `abrun` MCP surface (see "Security model" above for why).
