...
export function transitions(wf: WorkflowType): readonly TransitionRule[] {
...
export function canTransition(
  ...
  if (assignee === null || !rule.allowedAssignees.includes(assignee)) {
    return {
      ok: false,
      reason: `assignee_role '${String(assignee)}' not allowed for ${to}`,
    };
  }
  if (!rule.byRoles.includes(by)) {
    return {
      ok: false,
      reason: `role '${by}' cannot perform ${from} -> ${to}`,
    };
  }
  return { ok: true };
}

export function allowedPrevStatuses(
  wf: WorkflowType,
  to: TaskStatus,
): TaskStatus[] {
