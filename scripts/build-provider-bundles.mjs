...
const bundles = [
  {
    entryPoint: join(root, "plugins", "providers", "copilot", "src", "runner.ts"),
    outfile: join(
      root,
      "plugins",
      "claude-code",
...
for (const bundle of bundles) {
  ...
  if (output === undefined) throw new Error(`No bundle output for ${bundle.entryPoint}`);

  if (check) {
    const current = await readFile(bundle.outfile).catch(() => null);
    if (current === null || !current.equals(output)) {
      console.error(`${bundle.outfile} is stale; run npm run build:provider-bundles`);
      stale = true;
    }
    continue;
  }
